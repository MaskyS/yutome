...
billing_customers = Table(
    ...
    Column("workspace_id", Text, ForeignKey("workspaces.id"), nullable=False),
    Column("provider", Text, nullable=False),
    Column("external_customer_id", Text, nullable=False),
    ...
    UniqueConstraint("provider", "external_customer_id"),
)
...
credit_ledger_entries = Table(
    ...
    Column("external_customer_id", Text),
...
billing_exports = Table(
    ...
    Column("reservation_id", Text, ForeignKey("usage_reservations.id")),
    Column("billing_customer_id", Text, ForeignKey("billing_customers.id")),
    Column("price_book_id", Text, ForeignKey("price_books.id")),
    Column("provider", Text, nullable=False),
    Column("external_customer_id", Text),
    Column("customer_id", Text),
    Column("external_meter_key", Text),
    Column("external_event_id", Text),
    Column("event_name", Text, nullable=False),
    Column("export_units_jsonb", JSONB, nullable=False, server_default=text("'{}'::jsonb")),
    Column("source_event_dedupe_key", Text, nullable=False),
    Column("status", Text, nullable=False, server_default=text("'pending'")),
    Column("authorization_effect", Text, nullable=False, server_default=text("'none'")),
    ...
    Column("updated_at", DateTime(timezone=True), nullable=False, server_default=text("now()")),
    UniqueConstraint("provider", "source_event_dedupe_key"),
    UniqueConstraint("provider", "external_event_id"),
)

Index(
    "idx_billing_exports_replay",
    billing_exports.c.status,
    billing_exports.c.updated_at,
    postgresql_where=billing_exports.c.status.in_(["pending", "failed"]),
)

polar_webhook_snapshots = Table(
    "polar_webhook_snapshots",
    hosted_metadata,
    Column("id", Text, primary_key=True),
    Column("webhook_event_id", Text),
    Column("payload_hash", Text, nullable=False),
    Column("event_type", Text, nullable=False),
    Column("workspace_id", Text, ForeignKey("workspaces.id")),
    Column("external_customer_id", Text),
    Column("external_subscription_id", Text),
    Column("customer_state_snapshot_jsonb", JSONB, nullable=False, server_default=text("'{}'::jsonb")),
    Column("payload_jsonb", JSONB, nullable=False, server_default=text("'{}'::jsonb")),
    Column("replay_status", Text, nullable=False, server_default=text("'pending'")),
...
Index(
    "idx_polar_webhook_snapshots_replay",
    polar_webhook_snapshots.c.replay_status,
    polar_webhook_snapshots.c.received_at,
    postgresql_where=polar_webhook_snapshots.c.replay_status.in_(["pending", "failed"]),
)
