...
from yutome.asr import transcribe_with_faster_whisper
...
from yutome.embeddings import EmbeddingStats, embed_pending_chunks
...
from yutome.quality_upgrade import QualityUpgradeStats, upgrade_active_transcripts
from yutome.store import (
...
from yutome.transcripts import normalize_transcript, write_transcript_artifacts
...
class SyncStats:
    ...
    cleanup_upgraded: int = 0
    cleanup_skipped_unchanged: int = 0
    cleanup_skipped_missing: int = 0
    cleanup_skipped_quality: int = 0
    cleanup_failed: int = 0
    cleanup_chunks_saved: int = 0
...
def _process_metadata(
    *,
    ...
    if stream_progress is not None:
        stream_progress(f"Metadata {video.video_id}: {video.title or '(untitled)'}")
    try:
        _fetch_metadata_block(
            video=video,
            config=config,
            paths=paths,
            metadata_proxy=metadata_proxy,
    ...
    except Exception as exc:  # noqa: BLE001 - metadata should not erase transcript success.
        progress(f"  metadata failed after {_elapsed(started_at)}: {exc}")
        return 0, 1
    progress(f"  metadata saved in {_elapsed(started_at)}")
    return 1, 0
...
def sync_channel(
    *,
    ...
    if (
        ...
        raise ValueError("Webshare proxy is enabled but YUTOME_WEBSHARE_USERNAME/YUTOME_WEBSHARE_PASSWORD are missing.")
    ...
    if refresh_discovery:
        ...
        with connect_catalog(paths.catalog_db) as connection:
            discovered = list_catalog_videos(
                connection,
                channel_selector=None if target == "catalog" else target,
            )
    stats = {
    ...
    def run_candidates(
        ...
        *,
        ...
        phase_fetch_metadata: bool,
        collect_staged_retry: bool = False,
    ) -> None:
        phase_started_at = time.monotonic()
        phase_total = len(phase_candidates)
        phase_done = 0

        def report_phase_progress() -> None:
            if progress is None or phase_name is None or phase_total == 0:
                return
            elapsed = time.monotonic() - phase_started_at
            rate = phase_done / (elapsed / 60) if elapsed > 0 else 0.0
            remaining = phase_total - phase_done
            eta = (remaining / rate * 60) if rate > 0 and remaining else 0.0
            progress(
                f"{phase_name} progress: {phase_done}/{phase_total} complete; "
                f"transcripts={stats['transcripts_saved']} deferred={stats['deferred']} "
                f"failed={stats['failed']} elapsed={_format_duration(elapsed)} "
                f"rate={rate:.2f}/min eta={_format_duration(eta)}"
            )
        ...
        if progress and phase_name:
            progress(
                ...
                f"metadata {'enabled' if phase_fetch_metadata else 'deferred'}"
            )
        if workers <= 1 or not phase_candidates:
            for video, video_fallback_only in phase_candidates:
                video_result = _process_video(
                    ...
                    fetch_metadata=phase_fetch_metadata,
    ...
    run_candidates(
        ...
        phase_fetch_metadata=fetch_metadata and not staged_fallback,
    ...
    if staged_fallback:
        if rate_limit_stopped and stop_on_rate_limit and progress:
            progress("Stage 1 hit the rate-limit guard; continuing with staged fallback retry")
        rate_limit_stopped = False

    if staged_fallback and staged_retry_video_ids:
        if progress:
            progress(f"Stage 2 retrying {len(staged_retry_video_ids)} unresolved video(s) with yt-dlp")
    ...
    if staged_fallback:
        if rate_limit_stopped and stop_on_rate_limit and progress:
            progress("Stage 2 hit the rate-limit guard; continuing with exact metadata backfill")
        rate_limit_stopped = False

    if staged_fallback and attempted_video_ids:
        stats["processed"] = len(attempted_video_ids)
    ...
    if fetch_metadata and staged_fallback:
        metadata_ids = [
            video_id
            for video_id in attempted_video_ids
            if video_id in candidate_by_id
        ]
        metadata_candidates = [candidate_by_id[video_id][0] for video_id in sorted(metadata_ids)]
        if progress:
            progress(f"Stage 3/exact metadata backfill: {len(metadata_candidates)} candidate(s)")
        if metadata_candidates:
            progress_lock = Lock()

            def stream_metadata_progress(message: str) -> None:
                if progress is None:
                    return
                with progress_lock:
                    progress(message)
            ...
            if workers <= 1:
                for video in metadata_candidates:
                    saved, failed = _process_metadata(
                        video=video,
                        config=config,
                        paths=paths,
                        metadata_proxy=metadata_proxy,
                        stream_progress=stream_metadata_progress,
                    )
                    stats["metadata_saved"] += saved
                    stats["metadata_failed"] += failed
            else:
                max_workers = min(workers, len(metadata_candidates))
                with ThreadPoolExecutor(max_workers=max_workers) as executor:
                    futures = [
                        executor.submit(
                            _process_metadata,
    ...
    cleanup_video_ids = sorted(attempted_video_ids)
    if cleanup_video_ids and config.transcript_cleanup.enabled and config.transcript_cleanup.auto_after_sync:
        if config.gemini.enabled:
            if progress:
                progress(
                    f"Stage 4/heuristic transcript cleanup: scanning {len(cleanup_video_ids)} touched video(s)"
                )
            cleanup_stats: QualityUpgradeStats = upgrade_active_transcripts(
                config=config,
                paths=paths,
                video_ids=cleanup_video_ids,
                quality_gate=True,
                exclude_llm_cleanup=True,
                progress=progress,
            )
            stats["cleanup_scanned"] = cleanup_stats.scanned
            stats["cleanup_upgraded"] = cleanup_stats.upgraded
            stats["cleanup_skipped_unchanged"] = cleanup_stats.skipped_unchanged
            stats["cleanup_skipped_missing"] = cleanup_stats.skipped_missing
            stats["cleanup_skipped_quality"] = cleanup_stats.skipped_quality
            stats["cleanup_failed"] = cleanup_stats.failed
            stats["cleanup_chunks_saved"] = cleanup_stats.chunks_saved
            if progress:
                progress(
                    "Stage 4 complete: "
                    f"scanned={cleanup_stats.scanned} upgraded={cleanup_stats.upgraded} "
                    f"skipped_quality={cleanup_stats.skipped_quality} "
                    f"skipped_unchanged={cleanup_stats.skipped_unchanged} "
                    f"failed={cleanup_stats.failed}"
                )
        elif progress:
            progress(
                "Stage 4/heuristic transcript cleanup skipped: Gemini is not configured "
                "(set GEMINI_API_KEY or GOOGLE_API_KEY)."
            )
    elif cleanup_video_ids and progress and not config.transcript_cleanup.enabled:
        progress("Stage 4/heuristic transcript cleanup skipped: disabled in yutome.toml")
