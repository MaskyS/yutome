    ...
    *,
    ...
    if effective_worker and remove_cloudflare:
        ...
        if remove_cloudflare:
            _worker_deploy._delete_tracked_worker(effective_worker)
...
def setup(
    ...
    if not yes:
        node_ready = _worker_deploy._can_run_cloudflare_deploy()
        ...
        if connect_choice == skip_value:
            ...
            if not endpoint:
                ...
                try:
                    _save_deployed_worker_endpoint(
                        ...
                        endpoint=endpoint,
                        mode="connector_only",
                        worker_name=_worker_deploy.WORKER_PROJECT_NAME,
                    ...
                    _bridge._finalize_remote_bridge_setup(config_path=config, paths=paths)
        else:
            # Deploy path
            if not node_ready:
                typer.echo("")
            ...
            ) = _worker_deploy._deploy_tracked_worker(paths=paths)
            if deployed_url:
                _save_deployed_worker_endpoint(
                    config,
                ...
                _bridge._finalize_remote_bridge_setup(
                    config_path=config,
                    paths=paths,
...
def connect_command(
    ...
    if endpoint is None:
        ...
        if not deploy:
            ...
            typer.echo(f"  {_worker_deploy._tracked_worker_path()}")
            typer.echo("")
