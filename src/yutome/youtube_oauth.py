...
def load_or_authorize_token(
    *,
    ...
    if token and _token_is_valid(token):
        if status_callback:
            status_callback("Using cached YouTube OAuth token.")
        return token
    if token and token.get("refresh_token"):
        if status_callback:
            status_callback("Refreshing cached YouTube OAuth token.")
    ...
    if status_callback:
        if open_browser:
            status_callback("Opening YouTube OAuth consent for read-only subscription access.")
        else:
            status_callback("Printing YouTube OAuth consent URL for read-only subscription access.")
