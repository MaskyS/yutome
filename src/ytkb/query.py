...
from ytkb.retrieval import _format_hit, _snippet, _youtube_url
...
def _execute_lance_chunk(
    ...
    *,
    ...
    if not _lancedb_has_table(db, LANCEDB_CHUNKS_TABLE):
        raise RuntimeError("LanceDB chunks table is missing; run `ytkb rebuild-vectors`")
    table = db.open_table(LANCEDB_CHUNKS_TABLE)
    missing = _lancedb_missing_chunk_columns(table)
    if missing:
        missing_text = ", ".join(sorted(missing))
        raise RuntimeError(f"LanceDB chunks table is stale; missing {missing_text}. Run `ytkb rebuild-vectors`")
...
def _lance_search(table: Any, vector: list[float], text: str, mode: SearchMode, where: str, limit: int) -> list[dict[str, Any]]:
    try:
        ...
        if mode == "hybrid":
            raise RuntimeError(f"LanceDB hybrid search is not ready; run `ytkb rebuild-vectors`: {exc}") from exc
        raise
...
def _project_video_row(row: dict[str, Any], *, project: ProjectionName) -> dict[str, Any]:
    card = {
        ...
        "resource_uri": f"ytkb://video/{row['video_id']}",
        "title": row.get("title"),
        ...
        "active_transcript": (
            ...
            if row.get("active_transcript_id") is None
                ...
                "resource_uri": f"ytkb://transcript/{row['active_transcript_id']}",
            }
        ),
    }
    if row.get("score") is not None:
...
def _project_channel_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "resource_uri": f"ytkb://channel/{channel_id}" if channel_id else None,
