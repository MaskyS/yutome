...
echo "▶ Starting disposable Postgres ($CONTAINER) on :55432…"
docker rm -f "$CONTAINER" >/dev/null 2>&1 || true
docker run -d --rm --name "$CONTAINER" -e POSTGRES_PASSWORD=postgres -p 55432:5432 postgres:16 >/dev/null

echo "▶ Starting hosted API on :8000 (waits for Postgres, applies schema)…"
